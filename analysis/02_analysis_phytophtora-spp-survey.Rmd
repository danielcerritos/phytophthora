...
ggplot() +
  ...
  geom_sf(data = map_plant, aes(fill = samples_categ) , alpha = 0.6) +
...
isolates_recovered %>% 
  ...
  scale_fill_manual(values = c("#e31a1c", "#33a02c", "#1f78b4")) +
  theme_minimal_vgrid() +
...
isolates_species %>% 
  ...
  geom_col(alpha = 0.6, color = "gray68") +
  coord_flip() +
...
effective_rps <- sojae_rps %>% 
  ...
  geom_col(alpha = 0.6, fill =  "#529EFF", color = "gray68") +
  labs(x = "Rps gene", 
       y = "Percnet pathogenic") +
  scale_y_continuous(
...
complexity_mean <- complexity_percent %>% 
  ggplot(aes(x = complexity, y = percent)) +
  geom_col(alpha = 0.6, fill = "#F37B59", color = "gray68") + 
...
change the colors of this graphs to match the rest of the figures

```{r fig 3}
plot_grid(effective_rps, complexity_mean, labels = "AUTO", ncol = 1)
```
...
plot_grid(dis_length_log, dis_area_log) #improves
```

## Fit mixed model

```{r fm root dry }
# not nesting isolate:species, isolates coded in a way there is no redundancy
...
fig_length <- ggplot(data = aggressiveness_plots, aes(x = species, y = length_in, color =  species)) +
...
```{r figure 5}
fig_5A <-  ggplot(aggressiveness_plots, aes(x = reorder (isolate, root_mg), y = root_mg, color = species)) + 
  coord_flip() +
...
look the papers to see what graphs/table to do, distribution of EC50s based on color of species seems ok
look the book of agriculture stats with R, too see if add something for model fitting 
...
# lower limit fit at zero in LL.3
