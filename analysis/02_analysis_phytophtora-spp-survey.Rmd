...
soil_samples %>%
  ...
  geom_col(alpha = 0.6) +
  coord_flip() +
  labs(x = "", y = "Soil Samples", fill ="Year") +
  theme_minimal_vgrid() +
...
plant_samples$County %>%
  n_distinct()
```
```{r plant samples counties}
plant_subsamples <- plant_samples %>% 
  group_by(PC_ID, County) %>% 
  count() 

plant_samples_county <- plant_subsamples %>% 
  filter(!is.na(County)) %>% 
  dplyr::select(county = County) %>% 
  group_by(county) %>% 
  count()

plant_samples_county %>%
  ggplot(aes(x = fct_reorder(county, n), y = n)) +
  geom_col(alpha = 0.6) +
  coord_flip() +
  labs(x = "", y = "Plant Samples", fill ="Year") +
  theme_minimal_vgrid() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))
```
...
```{r exp isolates}
identified_isolates <-  isolates_recovered %>%
...
```
```{r species collected}
isolates_species <- identified_isolates %>% 
  ...
  mutate(percentage = (n/96))

isolates_species$species %>% 
  n_distinct()
```
```{r isolates per species}
isolates_species %>% 
...
pathotype_frequency %>% pandoc.table()
```
...
species_root_dry <- lmer(root_mg ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(species_root_dry) 
anova(species_root_dry)
emmeans(species_root_dry, ~ species) %>% 
  cld.emmGrid()
```

```{r fm shoot dry }
species_shoot_dry <- lmer(shoot_mg ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(species_shoot_dry) 
anova(species_shoot_dry)
emmeans(species_shoot_dry, ~ species) %>% 
  cld.emmGrid()
```

```{r fm root length }
# log transformation for normality and slight difference in variances
species_root_length <- lmer(log(length_in) ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(species_root_length ) # diagnostic plots look fine
anova(species_root_length )
emmeans(species_root_length , ~ species, type = "response") %>% # back-transform estimates
  cld.emmGrid()
```

```{r fm root area}
species_root_area <- lmer(log(area_in.2) ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(species_root_area) # transformation also worked 
anova(species_root_area)
emmeans(species_root_area, ~ species, type = "response") %>% 
  cld.emmGrid()
```

## Figure 3

```{r fig 4, fig.height=5, fig.width=6,  warning=FALSE}
...
fig_3B <- aggressiveness_plots %>% 
  ggplot(aes(x = species, y = area_in.2, color =  species)) +
  geom_boxplot(alpha = 0.6) +
  geom_point(position = position_jitter(), alpha = 0.15, color = "gray50") +
  labs(y = expression('Root area (in'^2*')'), 
       x = "")  + 
  theme_minimal() +
  theme(axis.text.x = element_blank()) 
...
emmeans(fm_shoot_dry_sojae, ~ isolate) %>% 
  cld.emmGrid()
```
```{r distribution sojae}
qplot(length_in, data = sojae_aggressiveness) +
  theme_minimal() # skew to the left, some transformation needed
qplot(area_in.2, data = sojae_aggressiveness) +
  theme_minimal() 
```

```{r length sojae}
# log transformation for normality and slight difference in variances
fm_root_length_sojae <- lmer(log(length_in) ~ isolate + (1|experiment), data = sojae_aggressiveness)
...
```{r area sojea}
# log transformation for normality and slight difference in variances
fm_root_area_sojae <- lmer(log(area_in.2) ~ isolate + (1|experiment), data = sojae_aggressiveness)
...
```{r distribution sansomeana}
qplot(length_in, data = sansomeana_aggressiveness) +
  theme_minimal() # skew to the left, some transformation needed
qplot(area_in.2, data = sansomeana_aggressiveness) +
  theme_minimal() 
```
...
```{r sansomeana length}
fm_root_length_sansomeana <- lmer(log(length_in) ~ isolate + (1|experiment), data = sansomeana_aggressiveness)
...
```{r sansomeana area}
fm_root_area_sansomeana <- lmer(log(area_in.2) ~ isolate + (1|experiment), data = sansomeana_aggressiveness)
...
back_transform_means <- means_aggressiveness %>% 
  filter(measurement %in% c("Root length", "Root area")) %>% 
...
```{r figure 4, fig.height= 8, fig.width=5}
# use back-transform means from emmeans output
# means bit different from raw data  
...
fig_4A <- means_aggressiveness %>% 
  filter(measurement == "Root length") %>% 
  ggplot(aes(x = reorder (isolate, estimate), y = estimate, color = species)) + 
  coord_flip() +
  geom_pointrange(aes(ymin = estimate - std.error, ymax = estimate + std.error), 
                  alpha = 0.6) +
  geom_hline(yintercept = 12.8, linetype="dashed",  size = 0.7) +
  theme_minimal() +
  theme(legend.position = "none", 
...
fig_4B <- means_aggressiveness %>% 
  filter(measurement == "Root area") %>% 
  ggplot(aes(x = reorder (isolate, estimate), y = estimate, color = species)) + 
  coord_flip() +
  geom_pointrange(aes(ymin = estimate - std.error, ymax = estimate + std.error), 
                  alpha = 0.6) +
  geom_hline(yintercept = 0.26, linetype="dashed",  size = 0.7) +
  theme_minimal() +
  theme(legend.position = "none", 
        axis.text.y  = element_text(size = rel(0.95))) +
  labs(x = "",
...
fig_4D <- aggressiveness_plots %>% 
  ggplot(aes(x = reorder (isolate, shoot_mg), y = shoot_mg, color = species)) + 
  coord_flip() +
  stat_summary(fun.data = "mean_se", aes(colour = species), alpha = 0.6) +
  geom_hline(yintercept = 47, linetype="dashed",  size = 0.7) +
  theme_minimal() +
  theme(legend.position = "none", 
        axis.text.y  = element_text(size = rel(0.95))) +
  labs(x = "",
...
```{r figure 5,fig.height=5, fig.width=5.5 }
