...
means_experiment_1_2 <- bind_rows(length_mean, root_mean, root_mean, shoot_mean) %>% 
...
# root length 
length_df <- data.frame(temperature = c("1 and 2", "3"), line = c(NA, 27))

aggressiveness_complexity %>% 
  filter(!is.na(length_cm)) %>% 
  ggplot(aes(x = reorder (isolate, length_cm), y = length_cm, fill = temperature, color = species)) + 
  stat_summary(fun.data = "mean_se", alpha = 0.7,  position = position_dodge(width = .75)) +
  theme(legend.position = "bottom", 
        axis.text.x=element_text(angle=45, hjust=1), 
        legend.margin=margin(t = -0.3, unit='cm')) +
  geom_hline(data = length_df, aes(yintercept = line), 
             linetype="dashed",  size = 0.7, color = "gray50") +
  scale_color_manual(values = c("coral1", "grey70", "cornflowerblue" )) +
  facet_wrap(vars(temperature), ncol = 1) +
  labs(x = "Isolate", 
       y = "Root length (cm)", 
       color = "") +
  guides(fill = FALSE)
...
aggressiveness_complexity %>% 
  filter(!is.na(length_cm)) %>% 
  ggplot(aes(x = reorder (isolate, length_cm), y = length_cm, color = temperature)) + 
  stat_summary(fun.data = "mean_se", alpha = 0.7,  position = position_dodge(width = .75)) +
...
# root area
area_df <- data.frame(temperature = c("1 and 2", "3"), line = c(1.8, 2.156))

aggressiveness_complexity %>% 
  filter(!is.na(area_cm.2)) %>% 
  ggplot(aes(x = reorder (isolate, area_cm.2), y = area_cm.2, fill = temperature, color = species)) + 
  stat_summary(fun.data = "mean_se", alpha = 0.7,  position = position_dodge(width = .75)) +
  theme(legend.position = "bottom", 
        axis.text.x=element_text(angle=45, hjust=1), 
        legend.margin=margin(t = -0.3, unit='cm')) +
...
# root dry weight 
root_df <- data.frame(temperature = c("1 and 2", "3"), line = c(25.5, 7.714))

aggressiveness_complexity %>% 
  filter(!is.na(root_mg)) %>% 
  ggplot(aes(x = reorder (isolate, root_mg), y = root_mg, fill = temperature, color = species)) + 
  stat_summary(fun.data = "mean_se", alpha = 0.7,  position = position_dodge(width = .75)) +
  theme(legend.position = "bottom", 
        axis.text.x=element_text(angle=45, hjust=1), 
        legend.margin=margin(t = -0.3, unit='cm')) +
...
# shoot dry weight   
shoot_df <- data.frame(temperature = c("1 and 2", "3"), line = c(47.5, 31.65))

aggressiveness_complexity %>% 
  filter(!is.na(shoot_mg)) %>% 
  ggplot(aes(x = reorder (isolate, shoot_mg), y = shoot_mg, fill = temperature, color = species)) + 
  stat_summary(fun.data = "mean_se", alpha = 0.7,  position = position_dodge(width = .75)) +
  theme(legend.position = "bottom", 
        axis.text.x=element_text(angle=45, hjust=1), 
        legend.margin=margin(t = -0.3, unit='cm')) +
