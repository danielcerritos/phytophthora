...
```{r}
qplot(length_cm, data = aggressiveness_1_2) +
...
fm_root_length <- lm(log(length_cm) ~ species + isolate + experiment, data = aggressiveness_both)
# diagnostics
...
fm_root_area <- lm(log(area_cm.2) ~ experiment, data = aggressiveness_both)
...
```{r fm root dry}
fm_root_dry <- lm(root_mg ~ species + isolate + experiment, data = aggressiveness_both)
...
no_outliers <- aggressiveness_both[-c(9, 40, 52, 129, 191), ]
```
...
The experiment factor is significant for all aggressiveness components. I will analyze it by experiment.
...
aggressiveness_complexity$species <- str_replace_all(aggressiveness_complexity$species, 
...
# use emmeans for isolate means plots 
...
means_aggressiveness %>% 
  ...
  labs(x = "", 
       ...
       color = "") +
  guides(fill = FALSE) +
  facet_grid(vars(agg_component), scales = "free_y")

# root area
b4 <- means_aggressiveness %>% 
  filter(agg_component == "area", 
         experiment == "1 and 2") %>% 
  ggplot(aes(x = reorder(isolate, estimate), y = estimate, color = species)) + 
  geom_hline(yintercept = 3.245, linetype="dashed",  size = 0.7, color = "grey70") +
  geom_hline(yintercept = 1.52, linetype="dashed",  size = 0.7, color = "black") +
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error , ymax = estimate + std.error), width = 0.2) +
  theme_minimal() +
  theme(legend.position = "bottom", 
...
# root dry weight 
c4 <- means_aggressiveness %>% 
  filter(agg_component == "root dry", 
         experiment == "1 and 2") %>% 
  ggplot(aes(x = reorder(isolate, estimate), y = estimate, color = species)) + 
  geom_hline(yintercept = 26, linetype="dashed",  size = 0.7, color = "grey70") +
  geom_hline(yintercept = 25.5, linetype="dashed",  size = 0.7, color = "black") +
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error , ymax = estimate + std.error), width = 0.2) +
  theme_minimal() +
  theme(legend.position = "bottom", 
...
# shoot dry weight   
d4 <- means_aggressiveness %>% 
  filter(agg_component == "shoot dry", 
         experiment == "1 and 2") %>% 
  ggplot(aes(x = reorder(isolate, estimate), y = estimate, color = species)) + 
  geom_hline(yintercept = 78.4, linetype="dashed",  size = 0.7, color = "grey70") +
  geom_hline(yintercept = 47.4, linetype="dashed",  size = 0.7, color = "black") +
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error , ymax = estimate + std.error), width = 0.2) +
  theme_minimal() +
  theme(legend.position = "bottom", 
...
figure_four_no_legend <- plot_grid(
  a4 + theme(legend.position="none"),
...
means_aggressiveness %>% 
  ...
  labs(x = "", 
       y = "", 
       color = "") +
  facet_wrap(vars(facets), labeller = label_parsed, ncol = 1, 
             strip.position = "left", scales = "free_y" ) +
  geom_hline(data = line_most_agg, aes(yintercept = value),
             linetype="dashed",  size = 0.7, color = "grey70") +
...
```{r}
aggressiveness_transfrom <- read_csv(here("output", "transform_data", "02_aggressiveness_transform.csv"))
sojae_pathotype <- read_csv(here("output", "results", "02_pathotype-psojae_results.csv"))
...
regression_sojae <- aggressiveness_transfrom %>% 
  filter(species == "Phytophthora sojae")

regression_sojae <- inner_join(regression_sojae, sojae_complexity, by = "isolate")
```

```{r}
# correlation
# experiment 1 and 2
exp_1 <- regression_sojae %>%
  filter(!experiment == 3) %>% 
  dplyr::select(Complexity, shoot_mg, root_mg, length_cm, area_cm.2) %>%
  as.matrix() %>%
  rcorr(type = "pearson")

corrplot(exp_1$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
```

```{r}
aggressiveness_complexity %>% 
...
```{r}
# experiment 3
exp_3 <- regression_sojae %>%
  filter(experiment == 3) %>% 
  dplyr::select(Complexity, shoot_mg, root_mg, length_cm, area_cm.2) %>%
  as.matrix() %>%
  rcorr(type = "pearson")

corrplot(exp_3$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
