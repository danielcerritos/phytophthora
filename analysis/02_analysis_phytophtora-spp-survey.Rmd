...
aggressiveness_complexity$isolate_id <- str_replace_all(aggressiveness_complexity$isolate_id, 
                                          c("CA" = "A", 
                                            "CN" = "N")
                                          )

aggressiveness_complexity <- aggressiveness_complexity %>% 
  mutate(temperature = case_when(
    experiment == 3 ~ "3",
    TRUE ~ "1 and 2"
  ))

aggressiveness_complexity <- aggressiveness_complexity [-c(204), ] # remove outlier for plots
```

```{r fig 3, fig.height=5, fig.width=6,  warning=FALSE, message=FALSE}
aggressiveness_complexity  %>% 
  ggplot(aes(x = species, y = length_cm, fill = species)) +
  geom_boxplot(alpha = 0.6) +
  geom_point(position = position_jitterdodge(0.2), alpha = 0.15, color = "gray50") +
  labs(x = "", 
       y = "Root length (cm)", 
       color = "") + 
  theme_bw() +
...
aggressiveness_complexity %>% 
  ggplot(aes(x = species, y = area_cm.2, fill = species )) +
  geom_boxplot(alpha = 0.6) +
  geom_point(position = position_jitterdodge(0.2), alpha = 0.15, color = "gray50") +
  labs(x = "", 
...
aggressiveness_complexity %>% 
  ggplot(aes(x = species, y = root_mg, fill = species )) +
  geom_boxplot(alpha = 0.6) +
  geom_point(position = position_jitterdodge(0.2), alpha = 0.15, color = "gray50") +
  labs(x = "", 
...
aggressiveness_complexity %>% 
  ggplot(aes(x = species, y = shoot_mg, fill = species )) +
  geom_boxplot(alpha = 0.6) +
  geom_point(position = position_jitterdodge(0.2), alpha = 0.15, color = "gray50") +
  labs(x = "", 
...
means_aggressiveness %>% 
  ...
  labs(x = "Isolate", 
       ...
       color = "") +
  guides(fill = FALSE) +
  geom_hline(yintercept = 25, linetype="dashed",  size = 0.7, color = "black") +
  geom_hline(yintercept = 50, linetype="dashed",  size = 0.7, color = "gray50") 


# root area
area_df <- data.frame(experiment = c("1 and 2", "3"), line = c(1.8, 2.156))
...
means_aggressiveness %>% 
  ...
  geom_hline(data = area_df, aes(yintercept = line), 
             linetype="dashed",  size = 0.7, color = "gray50") 


# second option  
means_aggressiveness %>% 
  filter(agg_component == "area") %>% 
  ggplot(aes(x = reorder(isolate, estimate), y = estimate, color = experiment)) + 
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error , ymax = estimate + std.error), width = 0.2) +
  theme(legend.position = "bottom", 
        axis.text.x=element_text(angle=45, hjust=1), 
  ...
  labs(x = "Isolate", 
       ...
       color = "") +
  guides(fill = FALSE) +
  gghighlight(estimate < 2.156)


# root dry weight 
root_df <- data.frame(experiment = c("1 and 2", "3"), line = c(25.5, 7.714))
...
means_aggressiveness %>% 
  ...
  guides(fill = FALSE) +
  facet_wrap (vars(experiment), ncol = 1) +
  geom_hline(data = root_df, aes(yintercept = line), 
             linetype="dashed",  size = 0.7, color = "gray50")
...
# shoot dry weight   
shoot_df <- data.frame(experiment = c("1 and 2", "3"), line = c(47.5, 31.65))
...
means_aggressiveness %>% 
  ...
  guides(fill = FALSE) +
  facet_wrap (vars(experiment), ncol = 1) +
  geom_hline(data = shoot_df, aes(yintercept = line), 
             linetype="dashed",  size = 0.7, color = "gray50")
