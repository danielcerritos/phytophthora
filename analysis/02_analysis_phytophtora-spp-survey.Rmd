...
table_one <- phytophthora_isolates %>% 
...
isolates_species %>% 
  filter(!species %in% c("Pythium spp.", "Phytophthora spp.")) %>% # no species after BLAST
  dplyr::select(species) %>% 
  n_distinct()
...
sansomeana_table_one <- table_one %>% 
  filter(species == "Phytophthora sansomeana") %>% 
  mutate(Pathotype = NA)

table_one_modified <- bind_rows(sansomeana_table_one, pahtotypes_isolate) %>% 
  arrange(species, year)
  
# write_csv(table_one_modified, path = here("output", "results", "02_identification_phytophthora-isolates_table-1.csv"))
...
Look isolates that cause disease in Rps3a and Rps 1c

```{r}
sansomeana %>% 
    filter(gene %in% c("3a", "1c"), 
           percent.susceptible >75)

diversity_sansomena <- do.call(calculate_diversities, sansomeana_rps)
...
aggressiveness_plots <- aggressiveness_plots %>% 
...
prueba <- prueba %>% 
  mutate(length_cm = `Root length`*2.54, 
         area_cm.2 = `Root area` *6.4516)
