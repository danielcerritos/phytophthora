...
# compare model lack of fit 
mod_fit <- function(model){
  p <- modelFit(model)
  p$`p value`[2]
}

prueba <- prueba %>% 
  mutate(ll3.fit = purrr::map(ll3.model, mod_fit), 
         ll4.fit = purrr::map(ll4.model, mod_fit), 
         ll3.aic = purrr::map(ll3.model, AIC), 
         ll4.aic = purrr::map(ll4.model, AIC))

prueba %>% 
  unnest(c(ll4.aic, ll3.aic))
