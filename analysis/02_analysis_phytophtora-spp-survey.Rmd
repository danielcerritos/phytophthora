...
```{r library}
# install.packages(c("googleway", "ggrepel","ggspatial", "libwgeom", "sf", 
# "rnaturalearth", "rnaturalearthdata", "hagis", "corrplot", "Hmisc", "drc"))
...
both_EC50$experiment <- as.factor(both_EC50$experiment)
both_EC50$isolate <- as.factor(both_EC50$isolate)
...
nest_pooled <- fun.sens %>% 
  group_by(fungicide, isolate) %>% 
  nest() 

pooled_model <- nest_pooled %>% 
  mutate(ll4.model = purrr::map(data, fit_LL4),
         ll4.fit = purrr::map(ll4.model, mselect), 
         abs.EC50 = purrr::map(ll4.model, abs_EC50)
         )

selection <- fm.combine %>% 
  dplyr::select(ll4.fit) %>% 
  gather(ll4.fit, key = "model", value = "model.fit") %>%   
  unnest_wider(model.fit) %>% 
  arrange(desc(fungicide, isolate, logLik, IC, `Lack of fit`, `Res var`))


pooled_EC50 <- fm.combine %>% 
...
```{r hormetic models}
# select only isolates with hormetic effects
hormesis <- fungicide %>% 
...
hormetic_models <- semi_join(nest_pooled, hormesis)
...
hormetic_models <- hormetic_models %>% 
  mutate(ll4.model = purrr::map(data, fit_LL4),
         ll4.fit = purrr::map(ll4.model, mselect), 
         )
...
hormetic_models <- hormetic_models %>% 
  mutate(ll4.model = purrr::map(data, fit_LL4),
         crs.model = purrr::map(data, fit_CRS),
...
hormetic_selection <- hormetic_models %>% 
  dplyr::select(ll4.fit, crs.fit, bc4.fit)

hormetic_selection <- hormetic_selection %>% 
  gather(ll4.fit,crs.fit, bc4.fit,  key = "model", value = "model.fit") %>%   
  unnest_wider(model.fit) %>% 
  arrange(fungicide, isolate, desc(IC))
```
