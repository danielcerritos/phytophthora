...
fig_4A <- aggressiveness_plots %>% 
  ...
  geom_hline(yintercept = 23.5, linetype="dashed",  size = 1) +
  geom_hline(yintercept = 13.5, linetype="dashed",  size = 0.7, color = "gray50") +
  theme_half_open() +
  theme(legend.position = "none", 
...
fig_4D <- means_aggressiveness %>% 
  ...
  geom_hline(yintercept = 1.7545, linetype="dashed",  size = 0.7, 
             yintercept = 1.7545, linetype="dashed",  size = 0.7) +
  theme_minimal() +
  theme(legend.position = "none", 
        axis.text.y  = element_text(size = rel(0.95))) +
  labs(x = "",
...
sojae_all_data %>% 
  ggplot(aes(x = Complexity, y = `Root area`)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm")

sojae_all_data %>% 
  ggplot(aes(x = as.factor(Complexity), y = `Root area`, color = isolate_id)) +
  geom_point(position = position_jitter(), alpha = 0.5) +
  theme_bw() +
  geom_text(aes(label = isolate_id)) +
  theme(legend.position = "none") +
  geom_hline(yintercept = 1.7545, linetype = "longdash") +
  geom_hline(yintercept = 1.3015, linetype = "dashed", color = "gray50") +
  geom_hline(yintercept = 2.1485, linetype = "dashed", color = "gray50")




  sojae_all_data %>% 
  ggplot(aes(x = Complexity, y = `Root dry weigth`)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm")
  
sojae_all_data %>% 
  ggplot(aes(x = Complexity, y = `Root length`)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm")

sojae_all_data %>% 
  ggplot(aes(x = Complexity, y = `Shoot dry weigth`)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm")
...
sojae_complexity <- sojae_complexity %>% 
...
regression_sojae$experiment <- as.factor(regression_sojae$experiment)

reg_length <- regression_sojae %>% 
  ggplot(aes(y = length_cm, x = Complexity, color = experiment))+
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm") +
  theme(legend.position = "none")+
  theme_bw()

reg_area <- regression_sojae %>% 
  ggplot(aes(y = area_cm.2, x = Complexity, color = experiment))+
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm") +
  theme(legend.position = "none")+
  theme_bw()

reg_root <- regression_sojae %>% 
  ggplot(aes(y = root_mg, x = Complexity, color = experiment))+
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm")  +
  theme(legend.position = "none") +
  theme_bw()

reg_shoot <- regression_sojae %>% 
  ggplot(aes(y = shoot_mg, x = Complexity, color = experiment))+
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm") +
...
complexity_agg <- cowplot::plot_grid(reg_length, reg_area, reg_root, reg_shoot )

reg_legend <- get_legend(reg_shoot + theme(legend.position = "bottom"))

plot_grid(complexity_agg, reg_legend)
...
regression_sansomeana <- aggressiveness_transfrom %>% 
  filter(!species == "Phytophthora sojae")

regression_sansomeana$experiment <- as.factor(regression_sansomeana$experiment)

regression_sansomeana %>% 
  ggplot(aes(y = shoot_mg, x = species))+
  geom_boxplot() +
  geom_point(position = position_jitter()) 
