...
ggplot() +
  ...
  geom_sf(data = map_plant, aes(fill = samples_categ) , alpha = 0.5) +
...
(complexity_mean <- complexity_percent %>% 
  ...
  geom_col(fill = "#1f78b4", alpha = 0.5) + 
...
fm_dry_root <- lmer(root_mg ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(fm_dry_root) 
anova(fm.root.dry)
```

```{r fm shoot dry }
fm_shoot_dry <- lmer(shoot.mg ~ species + isolate + (1|experiment), data = aggressiveness)
...
fm_root_length <- lm(log(length_in) ~ species + isolate + (1|experiment), data = aggressiveness)
resid_panel(fm_root_length) # transformation worked 
...
emmeans(fm_root_area, ~ species, type = "response") # back-transform estimates
