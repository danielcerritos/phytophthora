...
(table_one <- identified_isolates %>% 
  ...
  dplyr::select(species, isolate_id, county, year) %>% 
  arrange(year)
)
...
Can't save the plot using the corrplot package, I am going to recreate plot with ggcorrplot.

```{r corr plot 2, fig.height=5, fig.width=5}
library(ggcorrplot)
(corr_plot <- ggcorrplot(sojae_corr$r, hc.order = TRUE, outline.color = "white", type = "lower",  
           colors = c("#6D9EC1", "white", "#E46726"), lab = TRUE,p.mat = sojae_corr$P, 
           insig = "pch", pch = 4, pch.col = "white", pch.cex = 8, lab_size = 6) 
  )

# ggsave("correlationplot.tiff", plot = corr_plot, dpi = 300, path = here("docs", "figure"), width = 5, height = 5)
```


## Aggressiveness: subset data by species 

Subset data by species to see difference withinn the species.
