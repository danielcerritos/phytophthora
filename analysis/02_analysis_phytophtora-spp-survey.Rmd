...
pacman::p_load(tidyverse, here, sf, maps, rnaturalearth, rnaturalearthdata, 
               ...
               Hmisc, corrplot, drc, scales, broom, ggfortify)
```
...
agg_temperature_filtered %>% 
  ...
  labs(x = "Date", y = "Room temperature (°C)")
```


Can we combine experiments?

```{r plot by experiment, warning=FALSE}
...
```{r species by experiment}
ggplot(data = aggressiveness, aes(x = species, y = length_cm, color = species)) +
  geom_boxplot() +
  geom_point() +
  theme_light() +
  theme(legend.position = "none") +
...
ggplot(data = aggressiveness, aes(x = species, y = area_cm.2, color = species)) +
  geom_boxplot() +
  theme_light() +
...
ggplot(data = aggressiveness, aes(x = species, y = root_mg, color = species)) +
  geom_boxplot() +
  theme_light() +
  geom_point(position = position_jitterdodge()) +
  theme(legend.position = "none") +
...
ggplot(data = aggressiveness, aes(x = species, y = shoot_mg, color = species)) +
  theme_light() +
  geom_boxplot() +
  geom_point(position = position_jitterdodge()) +
  theme(legend.position = "none") +
...
Unequal variances in length_cm and area_cm.2. Results seem to vary depending experiment, so I would include experiment as a factor.

Experiment 1 and 2 seem more similar. 

```{r distribution}
qplot(length_cm, data = aggressiveness) +
  facet_wrap(~ experiment, ncol = 1) +
  theme_light()
  
qplot(area_cm.2, data = aggressiveness) +
  facet_wrap(~ experiment, ncol = 1) +
  theme_light()

qplot(root_mg, data = aggressiveness) +
  facet_wrap(~ experiment, ncol = 1) +
  theme_light()

qplot(shoot_mg, data = aggressiveness) +
  facet_wrap(~ experiment, ncol = 1) +
  theme_light()
```

## Fit lm models and ANOVAs

```{r fm root length}
# not nesting isolate:species, isolates coded in a way there is no need
fm_root_length <- lm(log(length_cm) ~ species + isolate + experiment, data = aggressiveness)
# diagnostics
autoplot(fm_root_length, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_root_length$residuals)
# anova and mean separation
anova(fm_root_length) 
emmeans(fm_root_length , ~ experiment, type = "response") %>%
  multcomp::cld()
```

```{r fm root area}
# fit again with transformation
fm_root_area <- lm(log(area_cm.2) ~ experiment, data = aggressiveness)
autoplot(fm_root_area, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_root_area$residuals)
anova(fm_root_area) 
emmeans(fm_root_area , ~ experiment, type = "response") %>% # back-transform estimates
  multcomp::cld()
```

```{r fm root dry}
fm_root_dry <- lm(root_mg ~ species + isolate + experiment, data = aggressiveness)
autoplot(fm_root_dry, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_root_dry$residuals)
anova(fm_root_dry)
...
```{r fm shoot dry}
fm_shoot_dry <- lm(shoot_mg ~ species + isolate + experiment, data = aggressiveness)
autoplot(fm_shoot_dry, which = 1:6, ncol = 3, label.size = 3)
anova(fm_shoot_dry)
emmeans(fm_shoot_dry, ~ experiment) %>% 
  multcomp::cld()
emmeans(fm_shoot_dry, ~ isolate) %>% 
  multcomp::cld()
```

The experiment factor is significant for all aggressiveness components. I will analyze first two experiments together and the third separate. 

## Analysis by experiment 

```{r by experiment data}
aggressiveness_1_and_2 <- aggressiveness %>% 
  filter(!experiment == 3)
```

### Experiment 1 and 2

```{r experiment 1 and 2}
# root length
fm_root_length_01 <- lmer(length_cm ~ species + isolate + (1|experiment), data = aggressiveness_1_and_2)
summary(fm_root_length_01)
# diagnostics
resid_panel(fm_root_length_01) 
# anova and post-hoc
anova(fm_root_length_01)
emmeans(fm_root_length_01, ~ species) %>% 
  multcomp::cld()
emmeans(fm_root_length_01, ~ isolate) %>% 
  multcomp::cld()

# root area
fm_root_area_01 <- lmer(area_cm.2 ~ species + isolate + (1|experiment), data = aggressiveness_1_and_2)
resid_panel(fm_root_area_01) # non-constant variance
# log-transformation
fm_root_area_01_log <- lmer(log(area_cm.2) ~ species + isolate + (1|experiment), data = aggressiveness_1_and_2)
resid_panel(fm_root_area_01_log)
summary(fm_root_area_01_log)
# anova
anova(fm_root_area_01_log)
emmeans(fm_root_area_01_log,  ~ species, type = "response") %>% 
  multcomp::cld()
emmeans(fm_root_area_01_log, ~ isolate, type = "response") %>%  
  multcomp::cld() 

# root dry weight
fm_root_dry_01 <- lmer(root_mg ~ species + isolate + (1|experiment), data = aggressiveness_1_and_2)
resid_panel(fm_root_dry_01)
anova(fm_root_dry_01)
emmeans(fm_root_dry_01, ~ species) %>% 
  multcomp::cld()
emmeans(fm_root_dry_01, ~ isolate) %>% 
  multcomp::cld()

# shoot dry weight
fm_shoot_dry_01 <- lmer(shoot_mg ~ species + isolate + (1|experiment), data = aggressiveness_1_and_2)
resid_panel(fm_shoot_dry_01)
anova(fm_shoot_dry_01)
emmeans(fm_shoot_dry_01, ~ species) %>% 
  multcomp::cld()
emmeans(fm_shoot_dry_01, ~ isolate) %>% 
  multcomp::cld()
```
...
```{r experiment 3 data}
aggressiveness_3 <- aggressiveness %>% 
  filter(experiment == 3)
```

```{r experiment 3}
fm_root_length_03 <- lm(length_cm  ~ species + isolate, data = aggressiveness_3)
autoplot(fm_root_length_03, which = 1:6, ncol = 3, label.size = 3)  # outlier
qplot(fm_root_length_03$residuals)

fm_root_area_03 <- lm(area_cm.2 ~ species + isolate, data = aggressiveness_3)
autoplot(fm_root_area_03, which = 1:6, ncol = 3, label.size = 3) 
qplot(fm_root_area_03$residuals) # outlier

fm_root_dry_03 <- lm(root_mg ~ species + isolate, data = aggressiveness_3)
autoplot(fm_root_dry_03, which = 1:6, ncol = 3, label.size = 3) 
qplot(fm_root_dry_03$residuals) # outliers

fm_shoot_dry_03 <- lm(shoot_mg ~ species + isolate, data = aggressiveness_3)
autoplot(fm_shoot_dry_03, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_shoot_dry_03$residuals) # outlier
```

Check the observation # 6, which is an outlier in all the models fitted.

```{r analysis with no outlier}
aggressiveness_3[6, ]
aggressiveness_3 %>% filter(isolate == "17PR009A.2") # very different from other two reps
aggressiveness_3_out <- aggressiveness_3[-c(6), ] # remove observation from data

# fit models without the outlier, let see if it influences the results
fm_root_length_03_out <- lm(length_cm ~ species + isolate, data = aggressiveness_3_out)
# compare anova results 
anova(fm_root_length_03) # with outlier
anova(fm_root_length_03_out) # without 
# isolate p-value changed
autoplot(fm_root_length_03_out, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_root_length_03_out$residuals)
# mean separation
emmeans(fm_root_length_03_out, ~ species) %>% 
  multcomp::cld()
emmeans(fm_root_length_03, ~ species) %>%
  multcomp::cld()
emmeans(fm_root_length_03_out, ~ isolate) %>% 
  multcomp::cld()
emmeans(fm_root_length_03, ~ isolate) %>% 
  multcomp::cld()

fm_root_area_03_out <- lm(area_cm.2 ~ species + isolate, data = aggressiveness_3_out)
# compare anova results 
anova(fm_root_area_03)
anova(fm_root_area_03_out)
autoplot(fm_root_area_03_out, which = 1:6, ncol = 3, label.size = 3) 
qplot(fm_root_area_03_out$residuals) 
emmeans(fm_root_area_03_out, ~ species) %>% 
  multcomp::cld()
emmeans(fm_root_area_03_out, ~ isolate) %>% 
  multcomp::cld()

fm_root_dry_03_out <- lm(root_mg ~ species + isolate, data = aggressiveness_3_out)
# compare anova results 
anova(fm_root_dry_03)
anova(fm_root_dry_03_out)
autoplot(fm_root_dry_03_out, which = 1:6, ncol = 3, label.size = 3) 
qplot(fm_root_dry_03_out$residuals) 
emmeans(fm_root_dry_03_out, ~ species) %>% 
  multcomp::cld()
emmeans(fm_root_dry_03_out, ~ isolate) %>% 
  multcomp::cld()
emmeans(fm_root_dry_03, ~ isolate) %>% 
  multcomp::cld()

# shoot dry weight  
fm_shoot_dry_03_out <- lm(shoot_mg ~ species + isolate, data = aggressiveness_3_out)
autoplot(fm_shoot_dry_03_out, which = 1:6, ncol = 3, label.size = 3)
qplot(fm_shoot_dry_03_out$residuals) # outlier
# compare results
anova(fm_shoot_dry_03)
anova(fm_shoot_dry_03_out)
emmeans(fm_shoot_dry_03_out, ~ species) %>% 
  multcomp::cld()
emmeans(fm_shoot_dry_03_out, ~ isolate) %>% 
  multcomp::cld()
emmeans(fm_shoot_dry_03, ~ isolate) %>% 
  multcomp::cld()
```

### Save results

```{r aggressiveness means}
# Experiment 1 and 2
length_mean <- emmeans(fm_root_length_01, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "length")

area_mean <- emmeans(fm_root_area_01_log, ~ isolate, type = "response") %>%  
  tidy() %>% 
  mutate(agg_component = "area") %>% 
  rename(estimate = response)

root_mean <- emmeans(fm_root_dry_01, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "root dry")
  
shoot_mean <- emmeans(fm_shoot_dry_01, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "shoot dry")

means_experiment_1_2 <- bind_rows(length_mean, area_mean, root_mean, shoot_mean) %>% 
  mutate(experiment = "1 and 2")
  
# Experiment 3
length_mean_3 <- emmeans(fm_root_length_03_out, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "length")

area_mean_3 <-emmeans(fm_root_area_03_out, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "area")

root_mean_3 <-emmeans(fm_root_dry_03_out, ~ isolate) %>%    
  tidy() %>% 
  mutate(agg_component = "root dry")

shoot_mean_3 <-emmeans(fm_shoot_dry_03_out, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component= "shoot dry")

means_experiment_3 <- bind_rows(length_mean_3, area_mean_3, root_mean_3, shoot_mean_3) %>% 
  mutate(experiment = "3")

# combine both 
aggressiveness_isolates <- bind_rows(means_experiment_1_2, means_experiment_3)
# write_csv(aggressiveness_isolates, here("output", "results", "02_aggressiveness-means-isolates_results.csv"))
```

I would focus on experiments 1 and 2, the third is different and also the environment (temperature)  
...
aggressiveness$species <- str_replace_all(aggressiveness$species, 
...
c3 <- aggressiveness %>% 
  ggplot(aes(x = species, y = root_mg, color =  species)) +
  geom_boxplot() +
  geom_point(position = position_jitter(), alpha = 0.15, color = "gray50") +
  labs(x = "", 
       y = "Root weight (mg)", 
...
d3 <- aggressiveness %>% 
  ggplot(aes(x = species, y = shoot_mg, color =  species)) +
  geom_boxplot() +
  geom_point(position = position_jitter(), alpha = 0.15, color = "gray50") +
  labs(x = "", 
...
legend_three <- get_legend(a3 + theme(legend.position = "bottom"))
...
```{r figure 4, fig 3, fig.height=6, fig.width=6.8,  warning=FALSE, message=FALSE}
means_aggressiveness <- read_csv(here("output", "results", "02_aggressiveness-means-isolates_results.csv"))

# change facets name
means_aggressiveness$facets <- factor(means_aggressiveness$agg_component, 
                                      labels = c( 
                                        "Root~area~(cm^{2})", 
                                        "Root~length~(cm)", 
                                        "Root~weight~(mg)", 
...
# sig difference lines 
line_most_agg <- data.frame(facets = c("Root~area~(cm^{2})", "Root~length~(cm)", 
                                       "Root~weight~(mg)", "Shoot~weight~(mg)"), 
                            value = c(3.245, 91.75, 26, 78.4))

line_control <- data.frame(facets = c("Root~area~(cm^{2})", "Root~length~(cm)", 
                                       "Root~weight~(mg)", "Shoot~weight~(mg)"), 
                           value = c(1.52, NA, 25, 47.4))

figure_four <- means_aggressiveness %>% 
  filter(experiment == "1 and 2") %>% 
  ggplot(aes(x = reorder(isolate, estimate), y = estimate, color = species)) + 
  geom_hline(data = line_most_agg, aes(yintercept = value),
             linetype="dashed",  size = 0.7, color = "gray70") +
  geom_hline(data = line_control, aes(yintercept = value),
             linetype="dashed",  size = 0.7, color = "red", alpha = 0.5) +
  ...
  labs(x = "", 
       y = "", 
       color = "") +
  facet_wrap(vars(facets), labeller = label_parsed, 
             strip.position = "left", scales = "free_y", ncol = 1) 


# ggsave("figure4.tiff", plot = figure_four, dpi = 300, path = here("docs", "figure"), width = 6, height = 6.8) 
```
  
```{r effects}
means_aggressiveness <- read_csv(here("output", "results", "02_aggressiveness-means-isolates_results.csv"))

means_aggressiveness$agg_component
            
effect_length <-  means_aggressiveness %>% 
  filter(experiment == "1 and 2", 
         agg_component == "length") %>% 
  mutate(effect = (estimate - 90.2)/90.2)

effect_area <-  means_aggressiveness %>% 
  filter(experiment == "1 and 2", 
         agg_component == "area") %>% 
  mutate(effect = (estimate - 4.76 )/ 4.76)

effect_root_weight <-  means_aggressiveness %>% 
  filter(experiment == "1 and 2", 
         agg_component == "root dry") %>% 
  mutate(effect = (estimate - 38.4)/ 38.4)

effect_shoot_weight <-  means_aggressiveness %>% 
  filter(experiment == "1 and 2", 
         agg_component == "shoot dry") %>% 
  mutate(effect = (estimate - 88.2)/ 88.2)
```
  
```{r}
## length
effect_shoot_weight %>% 
  filter(estimate > 46.9 , 
...
## Correlation b/w aggressiveness and complexity

```{r corr data}
# combine with pathotype data 
...
sojae_pathotype %>% view()

sojae_complexity <- sojae_pathotype %>% 
  dplyr::select(isolate_id, Complexity) %>% 
  rename(isolate = isolate_id)

aggressiveness_complexity <- left_join(aggressiveness, sojae_complexity, by = "isolate")
```

```{r correlation }
# correlation
# experiment 1 and 2
corr_agg_patho_1 <- aggressiveness_complexity %>%
  filter(species == "Phytophthora sojae",
         experiment == 1) %>% 
  dplyr::select(Complexity, shoot_mg, root_mg, length_cm, area_cm.2) %>%
  as.matrix() %>%
  rcorr(type = "pearson")

corrplot(corr_agg_patho_1$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
# p-values
corr_agg_patho$P
```

```{r}
corr_agg_patho_2 <- aggressiveness_complexity %>%
  filter(species == "Phytophthora sojae",
         experiment == 2) %>% 
  dplyr::select(Complexity, shoot_mg, root_mg, length_cm, area_cm.2) %>%
  as.matrix() %>%
...
corrplot(corr_agg_patho_2$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
# p-values
corr_agg_patho$P
```

```{r}
corr_agg_patho_3 <- aggressiveness_complexity %>%
  filter(species == "Phytophthora sojae",
         experiment == 3) %>% 
  dplyr::select(Complexity, shoot_mg, root_mg, length_cm, area_cm.2) %>%
  as.matrix() %>%
...
corrplot(corr_agg_patho_3$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
# p-values
corr_agg_patho$P
```
...
```{r corr box plot}
box_corr1 <- aggressiveness_complexity %>% 
  filter(!is.na(Complexity)) %>% 
  ggplot(aes(x = factor(Complexity), y = root_mg)) +
  geom_boxplot() +
...
box_corr2 <- aggressiveness_complexity %>% 
  filter(experiment == 3,
    !is.na(Complexity)) %>% 
  ggplot(aes(x = factor(Complexity), y = root_mg)) +
  geom_boxplot() +
...
plot_grid(box_corr1, box_corr2, ncol = 1, align = "v", rel_heights = c(1, 1.1))
``` 
...
# when combine data
hormetic_isolates_both <- clean_fungicide %>% 
  group_by(fungicide, isolate, dose) %>% 
  summarise(rel.growth = mean(rel.growth)) %>% 
  filter(rel.growth > 100, 
         !dose == 0) %>% 
  dplyr::select(fungicide, isolate)


hormetic_isolates_both %>% view()
...
Overall, LL.34 model better fit for most isolates
...
## Correlation b/w fungicide sensitivity, aggressiveness and complexity

```{r combine fungicide, agg and patho}
# combine all data
EC50_sojae <- fungicide_EC50 %>% 
  filter(species == "Phytophthora sojae") %>% 
  dplyr::select(fungicide, isolate_id, EC50, county) %>% 
  spread(fungicide, EC50)

sojae_agg_comp <- aggressiveness_complexity %>% 
  filter(species == "Phytophthora sojae", 
         !experiment == 3) %>% 
  dplyr::select(experiment, isolate, shoot_mg, root_mg, length_cm, area_cm.2, Complexity, )

sojae_all_data <- left_join(sojae_agg_comp, EC50_sojae, by = c("isolate" = "isolate_id"))
```

```{r corr all sojae data}
# correlation all data including NAs
corr_sojae_all <- sojae_all_data %>%
  dplyr::select(Complexity, azoxystrobin, ethaboxam, mefenoxam, metalaxyl, shoot_mg, root_mg, length_cm,
                area_cm.2) %>%
  as.matrix() %>%
  rcorr(type = "pearson")

corrplot(corr_sojae_all$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
# p-values
corr_sojae_all$P
...
corrplot(corr_sojae_all$r, method = "color", col = col(200),
         type = "upper", order = "hclust", number.cex = 1.5,
         addCoef.col = "black", # Add coefficient of correlation
         tl.col = "black", tl.srt = 90, # Text label color and rotation
         # Combine with significance
         p.mat = corr_sojae_all$P, sig.level = 0.05, insig = "blank", 
...
rojas_data <- read_csv("https://raw.githubusercontent.com/Chilverslab/Rojas_Survey_Phytopath_2016/master/data/clean/Isolates_11-12_final.csv")
...
## noel data isolation oomycete paper
noel_data <- read_csv("https://raw.githubusercontent.com/noelzach/Oomycete-Isolation-from-seed-treatments/master/GitHub/IsolationRecoveryV6.csv")

noel_data %>% 
  filter(location == "IL", 
         !`Highest BLAST taxonomy` == ".") %>% 
  group_by(`Highest BLAST taxonomy`) %>% 
  count() %>% 
  arrange(n)

# 1 sansomena, no sojae, dominated by pythium 
