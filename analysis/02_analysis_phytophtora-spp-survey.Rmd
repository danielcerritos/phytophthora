...
anova(fm_root_dry)
emmeans(fm_root_dry, ~ isolate|experiment) %>% 
  multcomp::cld()
```
...
# correlation
# experiment 1 and 2
corr_agg_patho <- aggressiveness_complexity %>%
  filter(species == "Phytophthora sojae",
         !experiment == 3) %>% 
...
corrplot(corr_agg_patho$r, method = "number", type = "upper", order = "hclust", tl.col = "black", diag = FALSE)
# p-values
corr_agg_patho$P
```
...
box_corr1 <- aggressiveness_complexity %>% 
  filter(!experiment == 3,
    !is.na(Complexity)) %>% 
  ggplot(aes(x = factor(Complexity), y = length_cm)) +
  geom_boxplot() +
