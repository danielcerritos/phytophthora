...
```{r fig 4 AB}
fig_length <- ggplot(data = plot_data, aes(x = species, y = length_in, fill = species)) +
...
```{r isolates plot}
dry_root <- means_aggressiveness %>% 
  filter(measurement == "dry_root", 
         !species == "Control None") 

root_length <- means_aggressiveness %>% 
  filter(measurement == "root_length",
         !species == "Control None")

a <- ggplot(data = root_length, mapping = aes(x = reorder(isolate, response), y = response)) + 
  geom_point(size = 3, aes(color = species)) +
  coord_flip() +
  theme_minimal() +
  geom_hline(yintercept = 20, linetype="dashed",  size = 0.7) +
  scale_color_manual(values = c("#ff7f00", "#a6cee3", "#1f78b4")) +
  theme(legend.position = "none") +
  labs(x = "Isolate ID",
       y = "Root length (in)",
       color = "")
  
b <- ggplot(data = dry_root, mapping = aes(x = reorder(isolate, estimate), y = estimate)) + 
  geom_point(size = 3, aes(color = species)) +
  coord_flip() +
  theme_minimal() +
  geom_hline(yintercept = 22.5, linetype="dashed",  size = 0.7) +
  scale_color_manual(values = c("#ff7f00", "#a6cee3", "#1f78b4")) +
  theme(legend.position = "none") +
  labs(x = "",
       y = "Root length (in)",
