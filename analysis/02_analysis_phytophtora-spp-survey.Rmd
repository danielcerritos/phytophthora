...
```{r EC50s}
# LL.3 model better fit for most isolates
# extract EC50s from LL.3 model
abs.EC50 <- function(models){
  ED(models, respLev = c(50), type = "absolute", interval = "delta")
}

both.models <- both.models %>% 
  mutate(ll3.EC50 = purrr::map(ll3.model, abs.EC50))

EC50 <- both.models %>% 
  dplyr::select(ll3.EC50) %>% 
  unnest_wider(ll3.EC50) %>% 
  rename(EC50 = ...1  , 
         std.error = ...2 ,
         lower = ...3, 
         upper = ...4)
