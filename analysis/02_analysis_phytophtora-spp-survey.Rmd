...
clean_fungicide <- left_join(clean_fungicide, control_fungicide)
...
```{r EC50 hormetic models}
# on isolate I can calculate an EC50 look why after
hormetic_models <- hormetic_models %>% 
  filter(!(fungicide == "ethaboxam" & experiment == 1 & isolate == 12)) %>% 
  mutate(bc4.EC50 = purrr::map(crs.model, abs_EC50))

hormetic_EC50 <- hormetic_models %>% 
  dplyr::select(bc4.EC50) %>% 
  unnest_wider(bc4.EC50) %>% 
  rename(EC50 = ...1  , 
         std.error = ...2 ,
         lower = ...3, 
         upper = ...4) %>% 
...
compare_EC50 <- semi_join(log_EC50, hormetic_isolates) %>% 
  mutate(model = "LL4") 

compare_EC50 <- bind_rows(compare_EC50, hormetic_EC50)

compare_EC50$experiment <- as.factor(compare_EC50$experiment)
compare_EC50$isolate <- as.factor(compare_EC50$isolate)

compare_EC50 %>% 
  filter(fungicide == "mefenoxam") %>% 
