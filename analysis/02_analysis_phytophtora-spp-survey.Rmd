...
pacman::p_load(tidyverse, here, sf, maps, rnaturalearth, rnaturalearthdata, 
               ...
               Hmisc, corrplot, drc, scales, broom, ggfortify, ggtext)
```
...
```{r sample map, fig.height=5, fig.width=5}
...
# ggsave("figure1.tiff", plot = figure_one, dpi = 300, path = here("docs", "figure"), width = 5, height = 5)
```
...
```{r fig 2, fig.height=5.5, fig.width=4, warning=FALSE}
...
# ggsave("figure2.tiff", plot = figure_two, dpi = 300, path = here("docs", "figure"), width = 4, height = 5.5)
```
...
```{r temperature agg}
agg_temperature %>% 
  ggplot(aes(x = date, y = room_temperature)) +
  geom_line(group = 1)  +
  theme_minimal() +
...
plot_grid(by_exp_shoot, by_exp_dry)
```
...
root_means <-  emmeans(fm_root_dry, ~ isolate) %>%
  tidy() %>% 
  mutate(agg_component = "Root weight")

shoot_means <- emmeans(fm_shoot_dry, ~ isolate) %>% 
  tidy() %>% 
  mutate(agg_component = "Shoot weight")

all_means <- bind_rows(root_means, shoot_means)

write_csv(all_means, here("output", "results", "02_aggressiveness-means-isolates_results.csv"))
```

## Figure 3

```{r agg data}
plot_aggressiveness <- aggressiveness
# combine the two controls for plotting
plot_aggressiveness$species <- str_replace_all(plot_aggressiveness$species, 
...
```{r fig 3, fig.height = 5, fig.width = 3.25,  warning=FALSE, message=FALSE}
fig_3a <- plot_aggressiveness %>% 
...
new_legend_three <- get_legend( fig_3a + theme(legend.position = "bottom"))

(figure_three <- plot_grid(new_fig3_no_legend, new_legend_three, ncol = 1, rel_heights = c(1, 0.1)))

# ggsave("figure3.tiff", plot = figure_three, dpi = 300, path = here("docs", "figure"), width = 3.25, height = 5) 
```

## Figure 4

```{r figure 4, fig.height=4.5, fig.width=7,  warning=FALSE, message=FALSE}
...
# significantly difference lines 
line_most_agg <- data.frame(facets = c("Root~weight~(mg)", "Shoot~weight~(mg)"), 
                            value = c(20.565, 63.65))

line_control <- data.frame(facets = c("Root~weight~(mg)", "Shoot~weight~(mg)"), 
                           value = c(23.53, 51.1))

# figure 4 
fig_4 <- means_aggressiveness %>% 
...
# ggsave("figure4.tiff", plot = fig_4, dpi = 300, path = here("docs", "figure"), width = 7, height = 4.5) 
```
...
```{r}
# effect sizes to write the results 
means_aggressiveness  %>% 
  filter(isolate == "Control Agar") # compare against agar control

root_effect <- means_aggressiveness  %>% 
  filter(agg_component == "Root weight") %>% 
  mutate(reduction = (34.0  - estimate)/34.0)

shoot_effect <- means_aggressiveness  %>%
  filter(agg_component == "Shoot weight") %>% 
  mutate(reduction = (83.5  - estimate)/83.5)
```
...
aggressiveness_trans <- read_csv(here("output", "transform_data", "02_aggressiveness_transform.csv")) 
...
aggressiveness_complexity <- left_join(aggressiveness_trans, sojae_complexity, by = "isolate")
```

```{r correlation }
corr_agg_patho <- aggressiveness_complexity %>%
  filter(species == "Phytophthora sojae") %>% 
  dplyr::select(Complexity, shoot_mg, root_mg) %>%
  as.matrix() %>%
  rcorr(type = "pearson")
...
```{r corr box plot, fig.height=4, fig.width=3.25, warning=FALSE, message=FALSE}
# box-plot with complexity 
...
box_corr2 <- aggressiveness_complexity %>% 
  filter(!is.na(Complexity)) %>% 
  ggplot(aes(x = factor(Complexity), y = shoot_mg)) +
  geom_boxplot() +
  geom_point(alpha = 0.2) +
  labs(x = "Complexity", 
       y = "Shoot weight (mg)") +
...
fig_5 <- plot_grid(box_corr1, box_corr2, ncol = 1, align = "v", rel_heights = c(1, 1.1))
fig_5
# ggsave("figure5.tiff", plot = fig_5, dpi = 300, path = here("docs", "figure"), width = 3.25, height = 4)
``` 
...
```{r figure 5,fig.height=4.5, fig.width=5, message=FALSE}
...
(figure_six <- ggplot(EC50_plot, aes(x = EC50, fill = species)) +
  ...
  theme_minimal() +
  panel_border() +
  scale_fill_discrete(labels = c("*P. sansomeana*", "*P. sojae*")) +
...
# ggsave("figure6.tiff", plot = figure_six, dpi = 300, path = here("docs", "figure"), width = 5, height = 4.5)
```
