...
samples.2018 <- read_excel(here("data", "Plant_Clinic_2018.xlsx"))
...
# aggressiveness 
temperature_files <- list.files(here("data"), full.names = TRUE, pattern = "temperature")
exp_names <- c(1, 2, 3)
...
length_area <- path.length.area %>% 
  set_names(exp_names) %>% 
  map_df(read_excel, skip = 1, .id = "names")

path.emer.damp <- list.files(here("data"), pattern = "germination", full.names = TRUE)
emergence.dampingoff <- path.emer.damp %>% 
  set_names(exp_names) %>% 
  map_df(read_csv, .id = "names")

# temperature data from greenhouse 
...
samples.2018 <- samples.2018 %>% 
  dplyr::select(PC_ID, Submit_Date = `Submit Date`, County, Genus_confirmation =`Genus Confirmation`, 
                PBNIC, plate_name = `Plate name`, morfological_assesment = `Morfological assesment`, 
                morfological_date = Date, days_store = `Days store`)
...
```{r agg data}
emergence <- emergence.dampingoff %>% 
  select(names, pot, emergence, germination)

damping_off <- emergence.dampingoff %>% 
  select(names, pot, damping_off.1:damping_off.6)

design_energence <- inner_join(agg.design, emergence, by = c("names", "pot")) 
most_data <- inner_join(design_energence, dry_weight, by = c("names", "pot"))
most_data <- most_data %>% 
  rename(experiment = "names")

write_csv(most_data, here("output", "clean_data", "01_agg_germination-dry-weigth_clean-data.csv"))

length_area <- length_area %>% 
  select(experiment = names, pot = `Sample #`,
         length_in = `Calib. Length inch`, 
         area_in.2 = area)

write_csv(length_area, path = here("output", "clean_data",
                                   "01_agg_length-area_clean-data.csv"))
...
```{r temperature agg}
experiment1 <- read_excel(temperature_files[1]) %>% 
  select(date_time = `Date/Time`, 
         room_temperature = `1713 Room temperature`, 
         heating_target = `heating target`, 
         cooling_target = `cooling target`,
         outside_temperature = `outside temperature`) %>% 
  mutate(experiment = 1)

experiment2 <- read_excel(temperature_files[2])  %>% 
  select(date_time = `Date/Time`, 
         room_temperature = `USDA 7W4 ROOM TEMP`, 
         heating_target = `USDA 7W4 HEAT TEMP`, 
         cooling_target = `USDA 7W4 VENT TEMP`,
         outside_temperature = `OUTSIDE TEMP`) %>% 
  mutate(experiment = 2)

experiment3 <- read_excel(temperature_files[3])  %>% 
  select(date_time = Date, 
         ...
         outside_temperature = `Outside Temperature`) %>% 
  mutate(experiment = 3) %>% 
  filter(!room_temperature == "NULL")

temperature_data <- rbind(experiment1, experiment2, experiment3) %>% 
  separate(col = date_time, into = c("date", "time"), sep = ",") 

temperature_data$room_temperature <- as.numeric(temperature_data$room_temperature)

write_csv(temperature_data, here("output", "clean_data", "01_agg_room-temperature_clean-data.csv"))
```
...
write_csv(fs.2018, path = here("output", "clean_data", "01_fungicide-sensitivity-2018_clean-data.csv"))

fs.2019 <- fs.2019 %>% 
  separate(names, sep = "_", into = c("fungicide", "experiment"))

write_csv(fs.2019, path = here("output", "clean_data", "01_fungicide-sensitivity-2019_clean-data.csv"))
```
